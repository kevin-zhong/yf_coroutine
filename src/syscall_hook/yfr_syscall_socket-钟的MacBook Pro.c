...
#define _yfr_syscall_socket_status_chck(fd, ctx, evt, r, type) \
        ...
        { \
                ...
                yf_log_error(YF_LOG_WARN, r->log, yf_socket_errno, \
                                "fd=%d %s status=%d,%d,%d", \
                                fd, _yfr_syscall_socket_desc[type], \
...
rw_again: \
        ...
        { \
                ...
                { \
                        ...
                        yf_log_debug2(YF_LOG_DEBUG, r->log, yf_socket_errno, \
...
#ifdef YFR_DEBUG
        yf_log_debug3(YF_LOG_DEBUG, r->log, 0, "recvfrom fd=%d blen=%d, flags=%d", 
                s, len, flags);
#endif
        ssize_t  rr = 0;
...
#ifdef YFR_DEBUG
        yf_log_debug3(YF_LOG_DEBUG, r->log, 0, "sendto fd=%d blen=%d, flags=%d", 
                s, len, flags);
#endif
        ssize_t  rr = 0;
...
#ifdef YFR_DEBUG
        yf_log_debug2(YF_LOG_DEBUG, r->log, 0, "sendmsg fd=%d flags=%d", 
                s, flags);
#endif
        ssize_t  rr = 0;
...
int socket(int domain, int type, int protocol)
{
        ...
        yf_log_debug4(YF_LOG_DEBUG, r->log, 0, 
                        "new socket fd=%d : {domain=%d, type=%d, protocol=%d}", 
...
#ifdef YFR_DEBUG
        yf_log_debug2(YF_LOG_DEBUG, r->log, 0, "poll in coroutine, nfds=%d, timeout=%d", 
                nfds, timeout);
#endif
        if (nfds == 0)
                return 0;
...
#ifdef YFR_DEBUG
        yf_log_debug1(YF_LOG_DEBUG, r->log, 0, "select in coroutine, nfds=%d", 
                nfds);
#endif
